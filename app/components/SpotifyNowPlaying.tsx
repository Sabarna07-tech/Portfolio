...
export default function SpotifyNowPlaying() {
  ...
  const progress =
    ...
    data.durationMs > 0
      ...
      : null;
  const profileUrl = 'https://open.spotify.com/user/sabarna07';
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.98 }}
      className="flex items-center gap-3 bg-surface-container-highest/60 backdrop-blur-3xl border border-white/10 p-3 rounded-2xl shadow-xl transition-all duration-300 group overflow-hidden relative max-w-[360px] w-full"
    >
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        aria-label="Connect on Spotify"
        className="relative z-10 shrink-0 px-3 py-1.5 rounded-full border border-[#1DB954]/40 text-[10px] font-semibold tracking-wide uppercase text-[#1DB954] hover:bg-[#1DB954]/15 transition-colors"
      >
        Connect
      </a>
    </motion.div>
